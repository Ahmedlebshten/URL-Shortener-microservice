...
Main Application: A URL Shortener web service that converts long URLs into short, shareable links. Built with Node.js, PostgreSQL, containerized with Docker, and deployed on Kubernetes with auto-scaling.

🏗️ Architecture
text
GitHub → Jenkins → Terraform (AWS EKS) → Security Scanning
                         ↓
              Docker Build + Trivy Scan
                         ↓
              ArgoCD (GitOps Deployment)
                         ↓
         URL Shortener Service on Kubernetes
                         ↓
...
Tech Stack: Terraform, AWS (EKS/EC2/VPC), Jenkins, Security Tools
...
Builds Docker image for URL Shortener application (Node.js + PostgreSQL)
...
Features: URL shortening, click tracking, custom aliases, QR codes

Tech Stack: Docker, Node.js, PostgreSQL, Jenkins, Trivy
...
Tech Stack: ArgoCD, Kubernetes, Helm, Prometheus, Grafana, Loki
...
Pre-deployment: Gitleaks (secrets), Trivy (IaC), SonarQube (code quality)
...
Alerts: Automated alerts on errors, high latency, or security issues
